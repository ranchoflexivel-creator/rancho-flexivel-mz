...
function getCount(){return Number(document.querySelector('#cartCount')?.textContent||0)}
function updateSticky(){const count=getCount();const el=document.querySelector('#rfStickyCartCount');if(el)el.textContent=`${count} ${count===1?'produto selecionado':'produtos selecionados'}`}
function openCart(){document.querySelector('#cartBtn')?.click()}
function markUi(){document.querySelectorAll('#productGrid article').forEach(card=>{const text=card.textContent||'';if(/promoção/i.test(text))card.querySelectorAll('span,div').forEach(e=>{if(/promoção/i.test(e.textContent||'')&&e.children.length===0)e.classList.add('rf-promo-highlight')});card.querySelectorAll('button').forEach(btn=>{if(/adicionar|add/i.test(btn.textContent||''))btn.classList.add('rf-add-highlight')})})}
function createSticky(){if(document.querySelector('#rfStickyCart'))return;const bar=document.createElement('div');bar.id='rfStickyCart';bar.setAttribute('aria-label','Resumo do pedido');bar.innerHTML='<div class="rf-sticky-count" id="rfStickyCartCount">0 produtos selecionados</div><button type="button" class="rf-sticky-button"><span class="material-symbols-outlined">shopping_cart</span><span>Ver pedido</span></button>';document.body.appendChild(bar);bar.querySelector('button').addEventListener('click',openCart);updateSticky()}
function addUnavailableDemo(){const grid=document.querySelector('#productGrid');if(!grid||grid.querySelector('[data-rf-unavailable-demo]'))return;const products=[['Produto de demonstração — Arroz','https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&w=700&q=80','5 kg'],['Produto de demonstração — Leite','https://images.unsplash.com/photo-1563636619-e9143da7973b?auto=format&fit=crop&w=700&q=80','1 L'],['Produto de demonstração — Refrigerante','https://images.unsplash.com/photo-1554866585-cd94860890b7?auto=format&fit=crop&w=700&q=80','2 L']];products.forEach(([name,image,unit])=>{const card=document.createElement('article');card.setAttribute('data-rf-unavailable-demo','true');card.className='bg-white rounded-2xl overflow-hidden border border-outline-variant shadow-sm';card.innerHTML=`<div class="relative h-[180px] overflow-hidden bg-surface-container-low"><img src="${image}" alt="${name}" class="w-full h-full object-cover opacity-70" loading="lazy"><span class="absolute top-3 left-3 rf-promo-highlight" style="background:#eef0ee!important;color:#5d655f!important;border-color:#c1c9bf!important">Indisponível</span></div><div class="p-4"><h3 class="font-bold text-sm">${name}</h3><div class="text-xs text-on-surface-variant mt-1">${unit}</div><button type="button" disabled class="rf-add-highlight rf-unavailable w-full mt-4 py-2.5 rounded-xl">Indisponível</button></div>`;grid.appendChild(card)})}
createSticky();
const observer=new MutationObserver(()=>{markUi();updateSticky()});observer.observe(document.body,{childList:true,subtree:true,characterData:true});
markUi();
setInterval(updateSticky,700);
setTimeout(addUnavailableDemo,1200);
