...
function writeCache(value){try{sessionStorage.setItem(CACHE_KEY,JSON.stringify(value));}catch{}}
async function loadProducts(){
  const cached=readCache();
  if(cached.length){products=cached;apply();return;}
  const {data,error}=await supabase.from('products').select('id,name,image_url,image,photo_url,imageUrl,active,stock').order('sort_order',{ascending:true});
...
function apply(){
  ...
  [...grid.children].forEach(card=>{
    ...
    img.alt=localText(product.name);img.loading='lazy';img.decoding='async';
    img.style.display='block';img.style.width='100%';img.style.height='100%';img.style.minHeight='180px';img.style.objectFit='cover';
  });
}
let scheduled=false;
